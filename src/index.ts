...
/**
 * Nevada Revised Statutes — state statutes by citation.
 *
 * Chapter files, section citations, same shape as Oregon and Texas: NRS
 * 200.010 sits inside NRS-200.html with the rest of chapter 200 (333,394
 * characters). Chapter is the part before the dot; the section is cut out
 * bounded by the next "NRS <chapter>.<n>" heading.
 *
 * Honest upstream: an unknown chapter returns 404.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Nevada Revised Statutes');
}

const BASE = 'https://www.leg.state.nv.us/NRS/NRS-';
const UA = 'pipeworx-mcp-nevada-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'nv_revised_statute',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "200.010" (murder) or "484C.110" (DUI). Chapter is the part before the dot.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'nv_revised_statute') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^NRS/i, '');
  const m = /^(\d+[A-Za-z]?)\.(\d+)$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Nevada Revised Statutes citation.`,
      hint: 'NRS sections are chapter.section, e.g. "200.010" (murder) or "484C.110" (DUI).' };
  }
  const chapter = m[1];
  const url = `${BASE}${chapter}.html`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'chapter_not_found', section: raw, chapter,
      message: `No chapter ${chapter} in the Nevada Revised Statutes (HTTP 404).`,
      hint: 'Chapter numbers are not contiguous — repealed chapters are normal.' };
  }
  if (!res.ok) throw new Error(`Nevada Legislature returned HTTP ${res.status} for ${raw}`);

  const text = statuteText(await readStatuteResponse(res));
  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const body = sliceSection(
    text,
    new RegExp(`NRS\\s*${esc}\\s`, 'g'),
    new RegExp(`NRS\\s*\\d+[A-Za-z]?\\.\\d+\\s`, 'g'),
  );
  if (!body) {
    return { found: false, reason: 'section_not_found', section: raw, chapter,
      message: `Chapter ${chapter} exists but has no section ${raw}.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }

  return {
    citation: `NRS ${raw}`, section: raw, chapter,
    text: body, text_chars: body.length, url,
    source: 'Nevada Legislature (leg.state.nv.us), keyless',
    attribution: 'Nevada statutes are public record.',
  };
}
